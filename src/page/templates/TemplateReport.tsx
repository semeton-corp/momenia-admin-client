...
import { formatCategoryAndTagName } from "./components/constants";
...
export default function TemplateReport() {
  ...
  const stats = {
    ...
    active: templates.filter((t) => getStatusLabel(t.status) === "active")
      .length,
    inactive: templates.filter((t) => getStatusLabel(t.status) === "inactive")
      .length,
  };
    ...
    <div className="px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <div className="text-3xl font-bold mt-2 text-gray-500">
            {stats.draft}
          </div>
        </div>
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <tbody>
            {templates.map((template) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-sm text-muted-foreground">
                  {formatApiDate(template.lastModifiedAt)}
                </td>
              </tr>
            ))}
          </tbody>
